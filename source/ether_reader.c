...
int ether_reader(const u_char *bytes, bpf_u_int32 totalLength)
{
  ...
  switch(ntohs(headerEthernet->ether_type))
  {
    case ETHERTYPE_IP:
      printf("\nETHERTYPE:  IP\n");
      ip_reader(bytes + sizeof(*headerEthernet), totalLength, sizeof(*headerEthernet));
      break;
    case ETHERTYPE_ARP:
      printf("\nETHERTYPE:  ARP\n");
      arp_reader(bytes + sizeof(*headerEthernet), totalLength, sizeof(*headerEthernet));
      break;
    case ETHERTYPE_IPV6:
      printf("\nETHERTYPE:  IPv6\n");
      ipv6_reader(bytes + sizeof(*headerEthernet), totalLength, sizeof(*headerEthernet));
      break;
  }
}
